...
switch ($Command.ToLower()) {
    ...
    "interactive" {
        ...
        if (Test-Path "docker-compose.yml") {
            ...
            docker run -it --rm --user root -v "${PWD}:/app/pdfs" -v "${PWD}/out:/app/out" $ImageName
        }
    }
    ...
    "run" {
        ...
        if (Test-Path "docker-compose.yml") {
            ...
            docker run --rm --user root -v "${PWD}:/app/pdfs" -v "${PWD}/out:/app/out" $ImageName "/app/pdfs/$PdfFile"
        }
    }
    ...
    "init" {
        ...
        if (Test-Path "docker-compose.yml") {
            ...
            docker run --rm --user root -v "${PWD}:/app/pdfs" -v "${PWD}/out:/app/out" $ImageName python -c "from docling.document_converter import DocumentConverter; DocumentConverter(); print('Modelos inicializados com sucesso!')"
        }
